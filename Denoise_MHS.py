...
def sample_y(i, j, Y, X):
    curr_state = Y[i, j]
    neighbors = [Y[i - 1, j], Y[i, j - 1], Y[i, j + 1], Y[i + 1, j]]
    curr_energy = -ITA * curr_state * X[i, j] - BETA * np.sum(curr_state * np.array(neighbors))
    proposed_state = -curr_state
    proposed_energy = -ITA * proposed_state * X[i, j] - BETA * np.sum(proposed_state * np.array(neighbors))
    acceptance_prob = min(1, np.exp(curr_energy - proposed_energy))
    accepted = np.random.rand() < acceptance_prob
    return proposed_state if accepted else curr_state
...
def save_image(denoised_image,i):
    ...
    plt.savefig(f'Output_MHS/data{i}dn.png')
    plt.close()
...
if __name__ == '__main__':
    ...
    total_samples = 1000
    burn_in_steps = 100
    SSIMscore = f"SSIMscor_mhs"


    for i in range(1,7):
        logfile = f"Output_MHS/log_energy{i}"
        ...
        compare(f'data/data{i}.png',f'Output_MHS/data{i}dn.png',i)